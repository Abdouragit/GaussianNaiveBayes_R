...
server <- function(input, output, session) {
  ...
  y_proba <- reactiveVal(NULL)
  matrix <- reactiveVal(NULL)
  perf <- reactiveVal(NULL)
  ...
  observeEvent(input$predictButton, {
    ...
    #NB <- trained_model() # Récupérer le modèle entraîné depuis trained_model
  ...
  observeEvent(input$evaluate, {
    ...
    matrix = NB$confusion_matrix(y_test, y_pred)
